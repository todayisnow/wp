...
function dwqa_breadcrumb() {
	...
	if ( $search ) {
		$output .= '<span class="dwqa-sep"> &rsaquo; </span>';
		$output .= sprintf( '<span class="dwqa-current">%1$s "%2$s"</span>', __( 'Showing search results for', 'dwqa' ), rawurldecode( $search ) );
	}

	if ( $author ) {
		$output .= '<span class="dwqa-sep"> &rsaquo; </span>';
		$output .= sprintf( '<span class="dwqa-current">%1$s "%2$s"</span>', __( 'Author', 'dwqa' ), rawurldecode( $author ) );
	}
