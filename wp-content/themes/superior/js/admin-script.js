...
//                    console.log(attachment);
                    ...
                    if (field_for_uploaded_file !== undefined) {
                        ...
                        if(attachment.type === 'image'){
                            ...
                            //title of photo
                            $('#item_title').val(attachment.title);

                            //description
                            $('#item_desc').val(attachment.description);
                            $('#attachment_id').val(attachment.id);
                            $('#item_image_thumb').val(attachment.sizes.thumbnail.url);

                            //fill sizes for retina logo
                            if($upload_input.is('#logo_image_high_dpi')){
                                $('#logo_image_high_dpi_sizes').val(attachment.width+'|'+attachment.height);
                            }
...
//                            console.log(llRegEx.exec(val),zoomRegEx.exec(val), typeRegEx.exec(val));
            ...
            custom_sidebars : function(){
                var sidebars = $('#a13-custom-sidebars');
                if(sidebars.length){
...
//                            var elem = this;
                        ...
                        fill_item_details = function(index){
                            ...
                            for(i = 0; field = keys[i],  i < size; i++){
                                ...
                                fields_part.find('[name="'+field+'"]').val(fields[field]);
                            }

                            //change type of item
                            fields_part
                                .find('input[name="item_type"]')
